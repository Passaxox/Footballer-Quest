 ...
 for (const difficulty of ["normal", "easy"]) for (const [profile, rate] of [["few", .25], ["medium", .5], ["many", .75], ["ko-stress", .5]]) {
  ...
  for (let iteration=0; iteration<300; iteration++) {
   ...
   for (let wave=1; wave<=20; wave++) {
    ...
    const enemy=boss ? wave+(rules.checkpoints[wave]?.levelOffset??rules.defaultBossLevelOffset) : engine.enemyLevel(wave,run.rulesetId);
    const v=scenarios.selectEncounterVersion(run.scenarioState.id,wave,tier,[],random);
    samples[wave-1].push({mean,max:Math.max(...levels),gap:enemy-mean,enemy,ko:run.team.filter(p=>!p.hp).length,scenario:run.scenarioState.id,tier:boss ? boss.ids.reduce((sum,id)=>sum+engine.byId(id).tier,0)/boss.ids.length : v?.encounterTier});
    ...
    if(boss || wave===1 || random()<rate) {
     ...
     for(let i=0;i<count;i++) run.team=engine.grantCombatXp(run.team,run.team[0].uid,enemy,!!boss,run.rulesetId).team;
  ...
  for(let wave=1;wave<=20;wave++) {
   ...
   rows.push({difficulty,profile,wave,teamMean:+avg("mean").toFixed(2),teamP10:+s.map(r=>r.mean).sort((a,b)=>a-b)[29].toFixed(2),teamP90:+s.map(r=>r.mean).sort((a,b)=>a-b)[269].toFixed(2),gapP90:+s.map(r=>r.gap).sort((a,b)=>a-b)[269].toFixed(2),teamMax:+avg("max").toFixed(2),enemyMean:+avg("enemy").toFixed(2),gap:+avg("gap").toFixed(2),ko:+avg("ko").toFixed(2),scenarioCounts:Object.fromEntries(scenarios.SCENARIOS.map(c=>[c.id,s.filter(r=>r.scenario===c.id).length])),enemyTierMean:+avg("tier").toFixed(2)});
  }
 }
