...
import { inspectPng, auditAssets, loadVersions, webpDimensions } from "../scripts/asset-audit.mjs";
...
test("cross-check detects missing, orphan, invalid, shared and non-square assets deterministically without writes",async()=>{
 ...
 try {
  ...
  assert.equal(result.summary.missing,1);assert.equal(result.summary.orphans,3);assert.equal(result.summary.invalidPng,1);
...
test("current catalog audit preserves all 44 legacy files and surfaces existing identity warnings",async()=>{
 ...
 assert.ok(report.files.find(f=>f.filename==="mark.png").issues.some(i=>i.code==="FORMAT_EXTENSION_MISMATCH"));
});
